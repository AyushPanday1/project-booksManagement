...
const { isEmpty, isValidISBN } = require("../validation/validation")
...
const createBook = async function (req, res) {
    try {
        ...
        if (!(isValidISBN(ISBN))) return res.status(400).send({ stauts: false, message: "ISBN is invalid, please provide ISBN with 13 digits or one hyphen symbol do not include 0 in first three digits for ex: 129-0879087670 or 1290879087670" })
...
const getBooksById = async function (req, res) {
    try {
        ...
        let obj = {
            _id: bookData._id, title: bookData.title, excerpt: bookData.excerpt, userId: bookData.userId, category: bookData.category, subcategory: bookData.subcategory, isDeleted: bookData.isDeleted, reviews: totalReviews.length, releasedAt: bookData.releasedAt, createdAt: bookData.createdAt,
