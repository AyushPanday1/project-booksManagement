...
const createUser = async function (req, res) {
    try {
        ...
        if (address) {
            if (!(isEmpty(address.pincode))) { return res.status(400).send({ stauts: false, message: "pincode is Empty" }) }
            if (address.pincode) {
                if (!(isValidPincode(address.pincode))) { return res.status(400).send({ status: false, message: "Invalid pincode" }) }
            }
        }
        if (address) {
            if (!(isEmpty(address.city))) { return res.status(400).send({ stauts: false, message: "city is Empty" }) }
            if (address.city) {
                if (!(isValidFullName(address.city))) { return res.status(400).send({ status: false, message: "Invalid city name" }) }
            }
        }
        if (address) {
            if (!(isEmpty(address.street))) { return res.status(400).send({ stauts: false, message: "street is Empty" }) }
        }
